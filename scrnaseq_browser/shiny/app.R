  ...
  list(
    ...
    get_obs_col = function(col) {
      ...
      if (res$type == "num") {
        ...
        lvls <- res$levels
        # Handle -1 from python (which becomes 0 here). 
        # R drops 0 indices, shortening the vector. We must convert 0 to NA.
        indices <- codes + 1
    ...
    get_expression_batch = function(genes, idx0, measurement="RNA", layer="X") {
      # IMPORTANT: backend expects `genes: List[str]`. With jsonlite::toJSON(auto_unbox=TRUE),
      # a length-1 character vector would be unboxed into a scalar string and cause HTTP 422.
...
# ---- Scatterplot widget ----
# Use the installed `reglScatterplot` package (interactive).
...
server <- function(input, output, session) {
    ...
    if (app_state$view == "database") {
      ...
      tagList(
        div(class="floating-panel",
            ...
            selectInput("p1_color", "Annotation", choices=c("solid", z_data$obs_keys), selected="Condition"),
            ...
            hr(),
            selectizeInput("violin_gene", "Violin Gene", choices=NULL),
            selectInput("violin_group", "Group By", choices=z_data$obs_keys, selected="Condition"),
            selectInput("violin_layer", "Violin Layer", choices=z_data$info$layers %||% c("X")),
        ...
        div(class="main-content",
            navset_underline(
              ...
              nav_panel("Violin",
                        fluidRow(
                          column(12, div(class="plot-wrapper", style="padding: 24px; margin-bottom: 20px;",
                                         plotOutput("violin_plot", height="400px")))
                        )
              )
            )
        )
  ...
  observeEvent(input$load_ds, {
    ...
    # Use server-side selectize for violin_gene as well to avoid performance warning
  ...
  observeEvent(list(input$gene_search, input$layer_select), {
    tryCatch({
      ...
      # After the UI updates (plots mount), push colors via JS message handler.
      session$onFlushed(function() {
  ...
  output$violin_plot <- renderPlot({
    ...
    # aquí no tenemos lasso/subset aún; asumimos "all cells"
    ...
    if (length(expr) != length(grp)) {
      # This shouldn't happen if backend is consistent, but let's be safe
